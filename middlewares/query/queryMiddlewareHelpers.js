...
const paginationHelper=async (model,query,req)=>{
    ...
    if(model.collection.collectionName==="answers"){
        console.log(await model.find().where({question:req.params.id}),"awaait")
    ...
    return {
        query:model.collection.collectionName!="answers" ? query.skip(startIndex).limit(limit):query.find({_id:req.params.id},{"answers":{$slice:[startIndex,endIndex]}}),
