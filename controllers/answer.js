...
const undoLikeAnswer=asyncHandler(async (req, res, next) => {
  ...
  const answer=await Answer.findById(answerId);
  if(!answer.likes.includes(userId)){
    return next(new CustomError("You have already not liked this answer"),400);
  }
...
module.exports={addAnswerToQuestion,getAllAnswersByQuestion,getSingleAnswer,editAnswer,deleteAnswer,likeAnswer,undoLikeAnswer}